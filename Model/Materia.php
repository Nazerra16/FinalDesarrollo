...
class Materia extends Conexion {
    ...
    public function eliminarMateriasPro()
    {
        $this->conectar();
        $pre = mysqli_prepare($this->con, "DELETE FROM profesor_materia WHERE materia_id = ?");
        $pre->bind_param("i", $this->id);
        $pre->execute();
    }

    public function eliminarMateriasAl()
    {
        $this->conectar();
        $pre = mysqli_prepare($this->con, "DELETE FROM alumno_materia WHERE materia_id = ?");
        $pre->bind_param("i", $this->id);
        $pre->execute();
    }
}